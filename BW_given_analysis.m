...
%     for i = 1:N
%         for j = 1:N
%             vars_trans(i,j) = var(trans_all.trans_all_given_BW(i, j, :)) .* 10^5;
%         end
%     end
...
%     for i = 1:N
%         for j = 1:K
%             vars_emis(i,j) = var(emis_all.emis_all_given_BW(i, j, :)) .* 10^5;
%         end
%     end
...
%     disp(vars_emis);
    ...
    trans_all = permute(trans_all.trans_all_given_BW, [2 1 3]);
    trans_all = reshape(trans_all, [9, 1000]);
    emis_all = permute(emis_all.emis_all_given_BW, [2 1 3]);
    ...
    filepath = strcat('data/', num2str(length), '_BW_given_trans_analysis.png');
    ...
    xlim([0 7])
    set(gca,'XTick',[0:1:7])
    filepath = strcat('data/', num2str(length), '_BW_given_emis_analysis.png');
    print(gcf, '-dpng', filepath);
    delete(gcf)   
end
