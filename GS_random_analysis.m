...
for l = 1:size(length_vector, 2)
    ...
    trans_all = permute(trans_all.trans_all_random_GS, [2 1 3]);
    trans_all = reshape(trans_all, [9, 10]);
    emis_all = permute(emis_all.emis_all_random_GS, [2 1 3]);
    ...
    filepath = strcat('data/', num2str(length), '_GS_random_trans_analysis.png');
    ...
    xlim([0 7])
    set(gca,'XTick',[0:1:7])
    filepath = strcat('data/', num2str(length), '_GS_random_emis_analysis.png');
    print(gcf, '-dpng', filepath);
    delete(gcf)   
end
